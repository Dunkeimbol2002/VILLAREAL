...
function limpiar(){ 
	...
	$("#total_venta").val("");
	$(".filas").remove();
	$("#_subtotal").html('S/. 0.00');
	$("#_impuesto").html('S/. 0.00 ');
	$("#total").html("S/. 0.00");
...
function agregarDetalle(idarticulo,articulo,stock,precio_venta){
	...
	if (stock>0) { 
		...
		if (idarticulo != "") {
			...
			'<td><input type="number" name="precio_venta[]" id="precio_venta[]" value="'+precio_venta+'"min="0"></td>'+
...
function calcularTotales() { 
	...
	$('#_subtotal').html("S/." + subtotal.toFixed(2));
	$('#_impuesto').html("S/." + (total - subtotal).toFixed(2));
	$("#total").html("S/." + total);
